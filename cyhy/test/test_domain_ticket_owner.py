...
def removal_data(database):
    ...
    for key, owner, hostname, is_open in rows:
        ticket = Document(
            _id=key, owner=owner, hostname=hostname, open=is_open,
            time_closed=None, events=[]
        )
        tickets[key] = ticket
...
@pytest.mark.parametrize("domains,closed_ids", [
    ...
    for key, ticket in tickets.items():
        if key in closed_ids:
            ...
            ticket.add_event.assert_called_once_with(
                ...
                "hostname moved out of scope", time=timestamp,
            )
        else:
